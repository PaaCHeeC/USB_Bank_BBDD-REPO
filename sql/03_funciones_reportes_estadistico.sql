...
AS $$
    SELECT
        ...
        COALESCE(cu_agg.saldo_total_cuenta, 0.00) AS "Saldo_Total",
    ...
    LEFT JOIN (
        ...
        movimientos_unificados AS (
            SELECT m.nro_cuenta_origen AS nro_cuenta, m.monto_egreso AS egreso, 0.00::numeric AS ingreso FROM movimientos_filtrados m
            UNION ALL
            SELECT m.nro_cuenta_destino AS nro_cuenta, 0.00::numeric AS egreso, m.monto_ingreso AS ingreso FROM movimientos_filtrados m
        )
        SELECT cu_mov.id_cliente, COALESCE(SUM(mu.ingreso), 0.00) AS total_ingresos, COALESCE(SUM(mu.egreso), 0.00) AS total_egresos
...
-- SELECT * FROM "usb_bank"."fn_reporte_estadistico"('2020-01-01', '2026-12-31');