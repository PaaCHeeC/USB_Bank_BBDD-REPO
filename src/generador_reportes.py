...
def main():
    ...
    #hacemos el mockdata, ojo con esto, no debe alterarse ni el tipo de dato 'DataFrame' ni el nomnre de la variable
    datos_clientes = pd.DataFrame({
        'ID_Cliente': ['C001', 'C002', 'C003', 'C004'],
        'Nombre': ['Empresa Alpha', 'Juan Perez', 'Maria Gomez', 'Corp Beta'],
        'Tipo_Cliente': ['Corporativo', 'Natural', 'Natural', 'Corporativo'],
        'Canal_Afiliacion': ['Web', 'App', 'Sucursal', 'Web'],
        'Fecha_Registro': pd.to_datetime(['2023-01-15', '2023-05-20', '2023-08-10', '2023-11-05']),
        'Total_Productos': [5, 2, 3, 8],
        'Saldo_Total': [1500000.00, 4500.50, 12000.75, 8500000.00]
    })

    datos_movimiento = pd.DataFrame({
        'ID_Transaccion': ['T1', 'T2', 'T3', 'T4', 'T5'],
        'ID_Cliente': ['C001', 'C001', 'C002', 'C003', 'C004'],
        'Cuenta': ['CTA-111', 'CTA-112', 'CTA-222', 'CTA-333', 'CTA-444'],
        'Tarjeta': ['TDC-01', 'Ninguna', 'TDD-02', 'TDC-03', 'Ninguna'],
        'Canal_Tx': ['Web', 'SWIFT', 'App', 'POS', 'Web'],
        'Tipo_Movimiento': ['Ingreso', 'Egreso', 'Ingreso', 'Egreso', 'Ingreso'],
        'Monto': [500000, 20000, 1500, 300, 2000000]
    })
    ...
    def generador_reportes_estadisticos(fecha_inicio=None, fecha_fin=None, tipo_cliente=None, canal=None):
        clientes_filtrado = datos_clientes.copy()
        ...
        columnas_reporte = ['ID_Cliente', 'Nombre', 'Tipo_Cliente', 'Total_Productos', 'Saldo_Total']
        ...
        with open(nombre_archivo, 'w', encoding='utf-8') as f:
            ...
            if df_final.empty:
                ...
                f.write("\n\n" + "-" * 80 + "\n")
                f.write(f"SALDO TOTAL: ${df_final['Saldo_Total'].sum():,.2f}\n")

        print(f"[TXT OK] Reporte estadístico generado en: {nombre_archivo}")
        

        #genero el pdf en cuestion
        etiqueta_tipo = tipo_cliente if tipo_cliente else "General"
    ...
    def generador_reportes_contables(agrupar_por: str):
        ...
        df_pivot = pd.pivot_table(datos_movimiento, values='Monto', index=columna_agrupacion, 
