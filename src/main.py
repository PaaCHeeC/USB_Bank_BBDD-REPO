...
def normalizar_estado_movimiento(estado_raw):
    """Normaliza etiquetas de estado provenientes de GUI/entrada libre."""
    estado = (estado_raw or "Todos").strip()
    if not estado:
        return "Todos"

    estado_lower = estado.lower()
    if estado_lower.startswith("estado "):
        estado = estado[7:].strip() or "Todos"
        estado_lower = estado.lower()
    ...
    return etiquetas.get(estado_lower, estado)
...
def obtener_datos_y_generar(tipo_reporte, filtros):
    ...
    try:
        ...
        if tipo_reporte == "estadistico":
            ...
            fecha_inicio = filtros.get("inicio") if filtros.get("inicio") else None
            fecha_fin = filtros.get("fin") if filtros.get("fin") else None
            estado_movimiento = normalizar_estado_movimiento(
                filtros.get("estado_movimiento")
            )

            query = (
                'SELECT * FROM "usb_bank"."fn_reporte_contable_detalle"(%s, %s, %s);'
            )
        ...
        if tipo_reporte == "estadistico":
            ...
            generador_reportes_contables(
                ...
                fecha_fin=filtros.get("fin"),
                estado_movimiento=normalizar_estado_movimiento(
                    filtros.get("estado_movimiento", "Todos")
                ),
            )
        elif tipo_reporte == "auditoria":
